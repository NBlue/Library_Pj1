...
class BooksController {
    ...
    addNew(req, res) {
        ...
        if (!errors.isEmpty() || IdAuthors.length === 0) {
    ...
    update(req, res) {
        ...
        if (!errors.isEmpty() || IdAuthors.length === 0) {
            ...
            if (!req.files || Object.keys(req.files).length === 0) {
                booksData.ImgBook = linkFileOlder;
                Update(booksData);
            } else {
                ...
                booksData.ImgBook = imgBook.name;
                try {
                    fs.unlinkSync(
                        __dirname.replace(
                            "app\\controllers",
            ...
            function Update(booksData) {
                modelBooks.updateBook(booksData, function (err, data) {
                    // Xóa data cũ
                    modelWrite.deleteWrite(req.params.id);
                    // Thêm data mới
                    for (let element of IdAuthors) {
                        let obData = {};
                        obData.IdBook = req.params.id;
                        obData.IdAuthor = Number(element);
                        console.log(obData);
                        modelWrite.addWrite(obData, function (err, data) {});
                    }
                    res.redirect("/admin/books");
                });
            }
    ...
    delete(req, res) {
        ...
        modelWrite.deleteWrite(req.params.id);
        modelBooks.deleteBook(req.params.id);
