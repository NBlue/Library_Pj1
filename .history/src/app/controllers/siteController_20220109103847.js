        ...
        if (!errors.isEmpty()) {
            ...
            modelSite.checkAccAdmin(req.body, function (err, data) {
                if (data != null) {
                    ...
                    //     let data = await modelSite.checkAccUser(req.body, "and LockStatus IS NOT NULL");
                    //     if(data !== null) res.render("site/login", {data, alert});
                    //     else {
                    //         data = await modelSite.checkAccUser(req.body)
                    //     }
                    ...
                    modelSite.checkAccUser(req.body, function (err, data) {
                        console.log(data);
                        if (data != null && data.LockStatus.length == null) {
                            req.session.context = data;
                            res.redirect("/");
                        } else if (data != null && data.LockStatus.length > 0) {
                            var alert = {
        ...
        async function Search() {
            ...
            if (data.length > 0) {
                ...
                if (data.length > 0) {
                    ...
                    res.render("user/user_home", { alert });
                }
            }
        }
        Search();
