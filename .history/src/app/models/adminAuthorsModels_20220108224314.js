...
module.exports = function () {
    ...
    this.findByName = async function (Name) {
        try {
            ...
            let data = await pool
                .request()
                .query(
                    "SELECT * FROM Authors WHERE Name Like N'%" + Name + "%'"
                );
    ...
    //  Top 4 tac gia có nhiều sách:
    this.getTopAuthor = async function (result) {
        var pool = await conn;
        var sqlString =
            "WITH AuthorForTop AS" +
            "(SELECT a.IdAuthor, a.ImgAuthor, a.Name, COUNT(w.IdBook) as 'Quantity' FROM Authors a, Write w WHERE a.IdAuthor = w.IdAuthor GROUP BY  a.IdAuthor, a.ImgAuthor, a.Name)" +
            "SELECT TOP(4) * FROM AuthorForTop ORDER BY AuthorForTop.Quantity DESC;  ";
        return await pool.request().query(sqlString, function (err, data) {
            if (data.recordset.length > 0) {
                result(null, data.recordset);
            } else {
