...
module.exports = function () {
    ...
    this.getBookDetail = async function (id, result) {
        var pool = await conn;
        var sqlString = "SELECT * FROM Book1 WHERE IdBook = @varId";
        return await pool
            .request()
    ...
    this.getAuthorToBook = async function (result) {
        var pool = await conn;
    ...
    this.addBook = async function (newData, result) {
        var pool = await conn;
        var sqlString =
            "INSERT INTO Book1 (Name, TypeBook, Review, Quantity, ImgBook, LikeBook) VALUES (@name, @typeBook, @review, @quantity, @imgBook, @likeBook)";
        return await pool
            .request()
    ...
    this.updateBook = async function (newData, result) {
        var pool = await conn;
        var sqlString =
            "UPDATE Book1 SET Name = @name, TypeBook = @typeBook, Review = @review, Quantity = @quantity, ImgBook = @imgBook, LikeBook = @likeBook WHERE IdBook = @varId";
        return await pool
            .request()
            .input("name", sql.NVarChar, newData.Name)
            .input("typeBook", sql.NVarChar, newData.TypeBook)
    ...
    this.deleteBook = async function (id, result) {
        var pool = await conn;
        var sqlString = "DELETE FROM Book1 WHERE IdBook = @varId";
        return await pool
            .request()
            .input("varId", sql.Int, id)
            .query(sqlString, function (err, data) {
