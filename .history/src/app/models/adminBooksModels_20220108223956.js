...
module.exports = function () {
    ...
    // tìm tên gần đúng
    this.findBookByLikeName = async function (Name) {
        console.log('SELECT * FROM Books WHERE Name Like N"%' + Name + '%"');
        try {
            let pool = await conn;
            let data = await pool
                .request()
                .query('SELECT * FROM Books WHERE Name Like N"%' + Name + '%"');
    ...
    // Lấy ra top 6 quyển sách có số lượng Nhiều
    ...
    // Lấy ra top  sách cho mỗi loại sách
    this.getTopBook = async function (number, whereTypeBook) {
        try {
            let pool = await conn;
            let data = await pool
                .request()
        ...
        //SELECT TOP(5) * FROM Books ORDER BY Quantity DESC
    };
    ...
    this.statisticBook = async function (type, where) {
        // type: DESC or ASC
        try {
            let pool = await conn;
            let data = await pool
                .request()
                .query(
                    "SELECT TOP(5) IdBook, Name, Quantity FROM Books " +
                        where +
                        "ORDER BY Quantity " +
                        type
                );
