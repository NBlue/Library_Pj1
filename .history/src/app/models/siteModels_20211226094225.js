...
module.exports = function () {
    ...
    this.updateInforUser = async function (newData, result) {
        var pool = await conn;
        var sqlString =
            "UPDATE Users SET Username = @username, Name = @name, Email = @email, Address = @address WHERE IdUser = @idUser";
        return await pool
            .request()
            .input("username", sql.VarChar, newData.Username)
    ...
    this.getBookBorrowing = async function (idUser) {
        try {
            ...
            let data = await pool
                ...
                .query(
                    "SELECT * FROM Books, Borrow, Users WHERE Books.IdBook = Borrow.IdBook AND Users.IdUser = Borrow.IdUser AND ReturnDateActual IS NULL AND Users.IdUser = @IdUser"
                );
            return data.recordset;
        } catch (err) {
            console.log(err);
