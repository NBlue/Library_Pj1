    ...
    this.getBookBorrowed = async function (IdUser) {
        try {
            ...
            let data = await pool
                ...
                .input("IdUser", sql.Int, IdUser)
                .query(
                    "SELECT DISTINCT Books.IdBook, Books.Name, Books.ImgBook, Users.IdUser FROM Books, Borrow, Users WHERE Books.IdBook = Borrow.IdBook AND Users.IdUser = Borrow.IdUser AND ReturnDateActual IS NOT NULL AND Users.IdUser = @IdUser"
                );
            return data.recordset;
        } catch (err) {
            console.log(err);
    ...
    this.updateLockStatus = async function (IdUser, dateNow, LockNumber) {
        try {
            ...
            let data = await pool
                .request()
                .input("IdUser", sql.Int, IdUser)
                .input("LockNumber", sql.Int, LockNumber + 1)
                .input("LockStatus", sql.VarChar, dateNow)
                .query(
                    "UPDATE Users SET LockStatus = @LockStatus, LockNumber = @LockNumber, Score = 0 WHERE IdUser = @IdUser"
                );
            return data.recordset;
        } catch (err) {
            console.log(err);
