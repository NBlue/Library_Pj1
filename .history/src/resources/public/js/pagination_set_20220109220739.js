...
const onFilterUser = () => {
    ...
    for (let i = 0; i < AdminUser.length; i++) {
        let Name = AdminUser[i].querySelector(".user__item-fullname .fullname");
        let name = Name.textContent || Name.innerHTML;
        let Username = AdminUser[i].querySelector(
            ".user__item-username .username"
        );
        let username = Username.textContent || Username.innerHTML;
        let Email = AdminUser[i].querySelector(".user__item-email .email");
        let email = Email.textContent || Email.innerHTML;
        let LockNumber = AdminUser[i].querySelector(
            ".user__item-lockNumber .lockNumber"
        );
        let lockNumber = LockNumber.textContent || LockNumber.innerHTML;
        // let Score = AdminUser[i].querySelector(
        //     ".section user__item-score .score"
        // );
        // let score = Score.textContent || Score.innerHTML;

        if (
            name.toUpperCase().includes(filterBox) ||
            username.toUpperCase().includes(filterBox) ||
            email.toUpperCase().includes(filterBox) ||
            lockNumber.toLocaleUpperCase().includes(filterBox) ||
            // score.toLocaleUpperCase().includes(filterBox)
        ) {
            AdminUser[i].style.display = "";
            userItemShow.push(AdminUser[i]);
        } else {
